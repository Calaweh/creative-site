...
selectionMatFolder.add(params, 'uPatchScale', 0.1, 15.0).name('Patch Density').onChange(v => {
    if(selectedCatalyst) selectedCatalyst.material.uniforms.uPatchScale.value = v;
});
selectionMatFolder.add(params, 'uMicroScale', 10.0, 500.0).name('Micro Detail').onChange(v => {
    if(selectedCatalyst) selectedCatalyst.material.uniforms.uMicroScale.value = v;
});
selectionMatFolder.add(params, 'uBumpIntensity', 0.0, 0.05).name('Frost Intensity').onChange(v => {
    if(selectedCatalyst) selectedCatalyst.material.uniforms.uBumpIntensity.value = v;
});
...
function syncGUIWithSelection() {
    ...
    // 3. Micro-Frost (NEW)
    params.uPatchScale = selectedCatalyst.material.uniforms.uPatchScale.value;
    params.uMicroScale = selectedCatalyst.material.uniforms.uMicroScale.value;
    params.uBumpIntensity = selectedCatalyst.material.uniforms.uBumpIntensity.value;

    params.uVerticalStretch = selectedCatalyst.material.uniforms.uVerticalStretch.value;
